---
title: "Using Package 'rbc' to Calculate Outcomes-Based Migration Statistics"
...
Package `rbc` provides functions for an outcomes-based classification of border crossings.  This vignette demonstrates the use of the functions, by calculating residence and migration statuses for a small made-up set of border crossings.
...
In addition to `rbc`, we need to load some packages from the [tidyverse](www.tidyverse.org).
...
We use some made-up data, which is included in the `rbc` package.

```{r}
crossings <- rbc::border_crossings %>%
  as_data_frame()
crossings
```
...
One final piece of data preparation is recoding the `direction` variable. Instead of the `direction` variable, functions in package `rbc` use a binary variable called `is_arrival`.
...
mig_histories <- rbc::border_crossings %>% 
  mutate(personId = make_person_id(key1 = name, key2 = passport_no),
